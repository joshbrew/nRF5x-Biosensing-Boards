...
void PWMController::init() 
{
    ...
    // Set the clock divider to control the PWM frequency
...
void PWMController::updateWrapValue(uint32_t pulseWidthUs, uint32_t periodUs) 
{
    ...
    _wrapValue = (RPI_PICO_CLOCK_FREQUENCY / PWM_CLOCK_DIVIDER) / (1000000 / periodUs);
    ...
    // Set the PWM duty cycle to control the pulse width
    pwm_set_gpio_level(_pwmPin, level);
}
